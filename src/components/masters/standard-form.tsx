...
export function StandardForm({ initialData, onSave }: StandardFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const apiData = {
      Std_Type: values.name,
      IsBlocked: values.isBlocked,
    };
    
    try {
      if (isEditMode) {
        await axios.put(`/api/mtcstandards/${initialData?.id}`, apiData);
        toast({
          title: "Standard Updated",
          description: `The record has been updated successfully.`,
        });
      } else {
        await axios.post(`/api/mtcstandards`, apiData);
        toast({
